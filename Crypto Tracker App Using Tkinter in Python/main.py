...
class AdvancedCryptoDashboard(ctk.CTk):
    ...
    def __init__(self):
        ...
        self.title("MASA CryptoPulse Tracker")
    ...
    def create_sidebar_controls(self):
        ...
        brand_label = ctk.CTkLabel(
            self.sidebar, text="CRYPTO TRACKER", font=ctk.CTkFont(size=18, weight="bold")
        )
        brand_label.pack(padx=20, pady=(30, 40))

        crypto_lbl = ctk.CTkLabel(
            self.sidebar, text="Select Crypto Asset:", font=ctk.CTkFont(size=12, weight="bold")
        )
        ...
        time_lbl = ctk.CTkLabel(
            self.sidebar, text="Select Timeframe:", font=ctk.CTkFont(size=12, weight="bold")
        )
    ...
    def create_header_panel(self):
        ...
        self.price_display = ctk.CTkLabel(
            self.metrics_box, text="$0.00", font=ctk.CTkFont(size=26, weight="bold")
        )
        self.price_display.pack(anchor="e")

        self.change_display = ctk.CTkLabel(
            self.metrics_box, text="0.00%", font=ctk.CTkFont(size=13, weight="bold")
        )
    ...
    def update_dashboard_data(self):
        ...
        if df is not None and not df.empty:
            fmt_str = (
                f"${self.current_price:,.4f}" if self.current_price < 2 else f"${self.current_price:,.2f}"
            )
            ...
            self.ax.fill_between(
                df["datetime"], df["price"], df["price"].min() * 0.995, color=trend_color, alpha=0.12
            )
